...
{
    public class UpdateCourseDescriptionHandler(ZaminServices zaminServices, ICourseCommandRepository repository) : CommandHandler<UpdateCourseDescriptionCommand>(zaminServices)
    {
        private readonly ICourseCommandRepository _repository = repository;

        public override async Task<CommandResult> Handle(UpdateCourseDescriptionCommand command)
        {
            ...
            var parameter = _zaminServices.MapperFacade.Map<UpdateCourseDescriptionCommand, UpdateDescriptionParameter>(command);
